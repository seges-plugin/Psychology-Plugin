...
name: ema-review
version: "1.0.0"
description: Use when a person asks about check-ins, how they have been doing over time, or patterns in energy, sleep, stress, or day-to-day experience. Psychology does not provide systematic EMA collection or trend analysis. Create a session-only context brief first; use a minimum selected authorized source only for a qualitative retrospective, and state its limits plainly.
triggers:
  - "review my check-ins"
...
Psychology does not currently provide systematic EMA collection, scheduled check-ins, or a trend-analysis tool.
...
Psychology material, the bootstrap must first call `psychology_get_consent_status()` and obtain a current-session source
...
| Recent saved notes | `psychology_journal_get_recent()` once, bounded by the host’s visible limit | Describe themes in the returned slice and disclose that it is limited. |
| A specific saved-note theme | `psychology_journal_search()` only for the stated theme | Compare the selected evidence without claiming a complete record. |
| Earlier completed results | `psychology_list_my_assessments()` once, smallest useful scope | Contrast confirmed result dates or summaries only when the returned data permits it. |
| Latest summary | `psychology_get_my_profile()` once | Use it as an explicitly selected background summary, not a time series. |
| Memory transparency | `psychology_journal_view_memory()` once | Explain what the account currently returns; do not turn it into an implicit all-history analysis. |
...
local-export flow. A write always requires a fresh explicit choice and a fresh `psychology_get_consent_status()` check.
