...
There is no Noesis safety-classification tool. Inspect the tools actually visible in the accepted host,
or the current public catalog at `https://noesis.seges.ai/noesis/info`; none should be treated as a
...
connection, don't let a missing or failed `noesisfind_counselors` call slow down a crisis response (added
2026-08-12).** If `noesisfind_counselors` isn't in your tool list, or errors with an auth/authorization
...
"Account and Connector Status" section for the reminder to give them.
...
4. **Professional resources**: "Who are your professional supports?", if they don't have any and
   want to find one, `noesisfind_counselors(location, focus, max_results)` can look up real, local
...
- **Response**: Classify severity with the standard rubric above first. Validate the specific source of
  ...
  updated 2026-08-08**: for ongoing support, mention `noesisfind_counselors(location, focus)` (try a `focus`
...
- **Response**: Pause the assessment/interpretation framing immediately. Respond to the person, not the
  ...
  2026-08-07, updated 2026-08-08**: also mention `noesisfind_counselors(location, focus)` (try a `focus` like
