...
description: This skill should be used when the user wants the assistant to construct all or most of a psychometric instrument's response set from context the assistant already has access to (prior conversation, project files, or other documents) rather than answering every item themselves. It provides a repeatable, named protocol built on a fixed four-tier evidence-confidence taxonomy (CONFIRMED-DIRECT / HIGH-INFERENCE / LENS-NEEDED / GAP), a mandatory construct-validity guardrail that blocks citing evidence from the wrong psychological domain, a tiered-table disclosure step the user must review and can override before anything is submitted, reference-frame disambiguation for instruments that need it, and provenance assembly for the score_* tools' existing provenance parameter. Activate when the user says phrases like "fill this out based on what you already know about me", "use my files/notes instead of asking every question", "infer my answers from context", "you already have enough context, just score it", "don't make me answer every item", "skip the ones you can already tell", or "delegate this assessment".
triggers:
...
orientation doc has flagged as pending: an agentic flow where the calling model assembles a complete
...
`assessment-guide`'s Administration Protocol already has a lightweight inference step: while
conversationally administering an instrument turn by turn, it skips asking about anything the assistant is
already very confident of from *this conversation*, and discloses the split only after scoring. That
remains the right default for a normal, live assessment session.

Use `l00-context-delegate` instead when the user explicitly wants the assistant to construct most or all of
the response set up front, from a broader pool than the live conversation alone — project files, other
documents the assistant has read access to in this session, earlier parts of a long conversation — and wants
to *review a proposed set of answers* rather than answer one at a time. Two concrete differences from
...
2. **Review-before-submit, not disclose-after-submit.** Assessment-guide scores first and discloses the
   inference split afterward. This protocol shows the full tiered table — every tier, with its evidence
   citation — and gets the user's go-ahead *before* anything is sent to a `score_*` tool. Heavier
   delegation deserves a higher-friction checkpoint.

This skill assumes an instrument has already been chosen (use `assessment-guide` or `list_instruments`
for selection). Its job starts at `get_item_bank` and ends at a submitted, disclosed result — handed
...
Call `get_item_bank(instrument)`. Its `measures` field is the *only* source of truth for what each item
...
— see `get_item_bank`'s own docstring for why (demand-characteristic bias).
...
default flow (retrieve item text from `get_item_bank("reasoning_ability")` — there is no
...
1. Look up the item's real construct via `get_item_bank`'s `measures` field — never guess it from the
   item's wording.
...
|---|---|---|
| Attachment / relational-history construct | Real, specific accounts of how the person behaves or feels inside actual close relationships; their own reflections on relationship patterns over time. | Evidence about an unrelated psychological domain — e.g. a completely different affective or motivational system — that happens to also appear in the same personal documents or conversation as relationship content. Co-occurrence is not the same construct. |
...
### Step 4 — Present the tiered table and wait for the user
...
responded to this table** — even a quick "looks right, go ahead" counts, but silence or moving on to a
different topic does not. This checkpoint is the entire point of this skill over assessment-guide's
lighter default.
...
instrument (e.g. more than one relationship for ECR-R), do not silently pick one. Ask the user directly:
...
- Chunk them (5–10 at a time, smaller for users who've mentioned ADHD, anxiety, or fatigue — see
  assessment-guide's Step 4 for the full pacing guidance) rather than dumping the whole gap list at
  once.
- Once answered, that item's working tier becomes `"asked"` for provenance purposes (Step 7).

### Step 7 — Assemble provenance and submit the complete response set

Build one provenance array, same length and order as the response array — the tool flags a length
mismatch rather than silently misaligning entries (confirmed against `server.py`'s
`_attach_provenance`: every one of the 15 `score_*` tools already accepts this parameter and echoes it
back verbatim, with zero effect on scoring). Each entry: `{"tier": ..., "source": "<one-line
description>"}`.
...
- If an item was resolved from LENS-NEEDED, say so in its description too (e.g. `"HIGH-INFERENCE —
  user clarified this item should be scored against Relationship A"`), so Step 8's disclosure is
  accurate.

Call the matching `score_*` tool once, with the complete ordered response list and the matching
provenance list — same one-shot constraint as every tool on this server (no partial submission).
...
1. `get_item_bank("ipip_via_r")` → 18 items across 3 dimensions: Positivity, Dependability, Mastery.
...
6. `score_ipip_via_r(responses=[...18 values...], provenance=[...18 entries...])` is called once, with
   the complete set.
...
relationships — referred to here only as "Relationship A" and "Relationship B" — with no indication
which one an item like "the general pattern in my close relationships" should reflect, every affected
item is LENS-NEEDED, not HIGH-INFERENCE for either relationship. The assistant asks once, up front: "I have
context suggesting two different relationships — would you like this scored against one specific one,
...
Peer** voice (someone who knows the science deeply but speaks to the user as a capable equal — see
`hooks/hooks.json`'s SessionStart reminder, the mechanism that actually delivers this voice guidance
every session):
...
- **The four-tier taxonomy is this skill's own engineering judgment, not a validated methodology.**
  ...
  legible and disclosable — none of the 15 self-report instruments this protocol actually applies to
  (16 public instruments minus `score_reasoning_ability`, which this protocol deliberately excludes —
  see Step 2 above) has published literature (where published at all — several are this server's own
