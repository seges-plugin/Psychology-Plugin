...
> Global Character Dimensions. *Journal of Personality Assessment*, 106(5), 665-680. The raw result-schema ID
> `score_ipip_via_r` identifies a returned record; it is not a public MCP invocation. The record covers
> **18 items, 1-5 scale, three dimensions**, Positivity, Dependability,
...
virtue clusters (Wisdom, Courage, Humanity, Justice, Temperance, Transcendence). **That is not what a
returned record with raw schema ID `score_ipip_via_r` reports.** This result schema represents the *compact, 18-item "Core Strengths" form*, purpose-
...
it. Psychology currently offers **dimension-level resolution only** (3 numbers), not strength-level resolution
