# ECR-R Interpretation, Experiences in Close social contexts-Revised
...
> The raw result-schema ID `score_ecr_r` identifies a returned record; it is not a public MCP invocation. The record covers 36 items, 1-7 scale, two continuous dimensions, **Anxiety** (18
> items) and **Avoidance** (18 items), reported as subscale means, plus an optional `reference_frame`
> naming which social context the responses were actually about.
...
social contexts: how much you worry about a partner's commitment and availability (**Anxiety**), and how
...
**A returned record with raw schema ID `score_ecr_r` reports the two continuous means and nothing else; it does not label a person "secure,"
...
that doesn't get absorbed into a social context, independence that can be a genuine strength, especially in
...
from the social context, which has its own trade-offs during conflict or separation.
...
- **Lower Anxiety + lower Avoidance** ("security-leaning"): comfortable both depending on a partner and
  ...
  satisfying social contexts in the research literature, but still a *pattern*, not a personal achievement
  or a moral high ground, and not immune to social context difficulty from other causes.
...
do shift, through new social contexts, therapy, or deliberate relational work. Never frame a score as a
permanent verdict on how someone is capable of relating.

## Reading `reference_frame`

If the person answered with a specific social context in mind ("my social context with my partner of 3
years") rather than "romantic partners in general," say so explicitly when presenting results, and note
that the same person can genuinely score differently about different social contexts or life stages. A score
about one specific social context is not the same thing as a general disposition, and treating it as
...
Attachment questions can surface real pain about a current or past social context. If that happens, treat it
...
- **No neurodivergence-specific norm.** Emerging research specifically studies attachment measures in
  ...
  norms account for.
- **Doesn't evaluate a social context's health or a partner's behavior**, it describes this respondent's own
  reported patterns, not a verdict on anyone else or on the social context as a whole.
