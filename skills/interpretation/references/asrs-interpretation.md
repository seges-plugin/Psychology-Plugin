...
> 245-256. Scored by `score_asrs_part_a` (noesis-mcp): 6 items, 0-4 frequency scale ("Never" to "Very
...
**inattentive-type** patterns, two track **hyperactive/impulsive-type** patterns. `score_asrs_part_a` does
not compute separate subscores for these two clusters (it returns one combined `positive_count` across all
...
step, `find_counselors(location, focus, max_results)` can help locate a real, local provider (psychiatrist
or clinical psychologist for a formal evaluation; see `safety-guidelines.md`'s referral table) — always
relay its `disclaimer` and `crisis_line_note` fields alongside any results.
