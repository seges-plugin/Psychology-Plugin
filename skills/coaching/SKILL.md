...
> hosted Noesis connector. There is no session-context store and no session-logging tool: coaching context
...
**If a Noesis tool is not visible in the current host, or a call fails with an authorization error, say so
...
3. Provide resources: ND-affirming therapist directories (see `references/safety-guidelines.md`), or,
   ...
   host-visible `noesisfind_counselors(location, focus, max_results)` capability. It is an outside-listings
...
> `noesisget_consent_status`, show the current source choices, and ask for an explicit per-session selection
