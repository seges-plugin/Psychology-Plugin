...
references:
  ...
  - path: "references/asrs-interpretation.md"
    description: "Strength-framed, explicitly non-diagnostic interpretation of raw result-schema ID score_asrs_part_a — a data ID, not a public MCP invocation — and its 6-item ADHD screener result"
  - path: "references/ecr-r-interpretation.md"
    description: "Strength-framed interpretation of raw result-schema ID score_ecr_r — a data ID, not a public MCP invocation — and its two attachment dimensions (Anxiety, Avoidance), no categorical style label"
  - path: "references/swls-interpretation.md"
    description: "Strength-framed interpretation of raw result-schema ID score_swls — a data ID, not a public MCP invocation — and its life-satisfaction bands"
  - path: "references/ipip-via-r-interpretation.md"
    description: "Strength-framed interpretation of raw result-schema ID score_ipip_via_r — a data ID, not a public MCP invocation — and its 3 global dimensions (Positivity, Dependability, Mastery), NOT the full 24-strength VIA"
  - path: "references/financial-calibration-interpretation.md"
    description: "Strength-framed interpretation of raw result-schema ID score_financial_calibration — a data ID, not a public MCP invocation — and its literacy + calibration sub-scores"
  - path: "references/wellness-tracking-interpretation.md"
    description: "Strength-framed, non-diagnostic interpretation of raw result-schema ID score_wellness_tracking — a data ID, not a public MCP invocation — and its sleepiness + medication-adherence components"
---
...
returned score record. If the person provides a result, use the pasted data. If they ask for a prior account
result, the bootstrap must first record their narrow "saved results" choice; then resolve the exact
visible `psychology_list_my_assessments` name in authenticated `tools/list` and call it once with the
...
`psychology_list_my_assessments()` is for completed results and is not an in-progress assessment-resume tool.
Never call a scoring tool to recreate a previous result, and never automatically read a profile, journal,
...
**If a tool this step needs (e.g. `psychology_domain_filtered_report`, `psychology_battery_aggregate`) isn't in your tool
...
result you can't actually get. Follow `skills/00-session-bootstrap/SKILL.md`'s connector-unavailable
boundary: say so plainly, use only visible or deliberately supplied material, and do not guess a tool result.
...
1. If you administered the assessment yourself, you already have the result dict from
   a returned score record (for example, one with the raw schema ID `score_big_five` or `score_pvq_rr`;
   neither is a public MCP invocation). If the user pastes in prior
...
2. For Big Five, there is no population-normed comparison at all; its returned record's raw schema ID is
   `score_big_five` (not an MCP invocation), and its
...
3. `psychology_describe_norm(centile, trait)` is a real tool for turning a genuine population centile into a
   plain-language level, but no visible public scoring tool currently hands Big Five a real centile
...
7. **Next steps**, Suggested follow-up assessments or coaching focus. If the person explicitly asks for
   ...
   `psychology_find_counselors(location, focus, max_results)` capability. It is an outside-listings lookup,
...
3. Do not invent a percentile, cutoff, or population comparison; `psychology_list_instruments`'s
